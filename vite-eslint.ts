...
// See https://github.com/vitejs/vite/issues/818#issuecomment-693199741
export default (
  options = { extNames: ['.js', '.jsx', '.ts', '.tsx'] }
) => {
  return {
    transforms: [
      {
        test({ path: filepath }: { path: string }) {
          return (
            filepath.startsWith(cwd) && filepath.indexOf('node_modules') < 0 && options.extNames.includes(path.extname(filepath))
          );
        },
        async transform({
          code,
